...
import { fetchOrderedDishesData, updateDishState } from '../../utils/firestore';

const OrderedDishesScreen = ({ navigation, route }) => {
const [dishList, setDishList] = useState([]);
    ...
    const reloadDish = useCallback(async () => {
        try {
            const newData = await fetchOrderedDishesData();
            setDishList(newData);
    ...
    const handelChangeState = (itemId, itemState) => {
        ...
        if (itemState === "ordered") {
            newState = "preparing";
            notification = "Bạn có chắc muốn thay đổi trạng thái món ăn thành 'Đang chuẩn bị' không?"
        } else if (itemState === "preparing") {
            newState = "cooked";
            notification = "Bạn có chắc muốn thay đổi trạng thái món ăn thành 'Chờ phục vụ' không?"; // Màu xanh lá cho trạng thái "preparing"
        }
        Alert.alert(
            ...
            [
                ...
                {text: 'OK', style: 'default', onPress: () => {
                    updateDishState(itemId, newState)
                        .then(() => reloadDish())
    ...
    const renderDish = ({ item }) => {
        let ComponentType = user === "kitchen_manager" ? TouchableOpacity : View;
        let iconColor = "black";
    ...
    return (
        <Animated.View style={[styles.container, { transform: [{ translateY: animatedContainerRef }] }]}>
            <HomeHeadNav navigation={navigation} title='MÓN ĐÃ ĐẶT' user={user} />
            <View style={styles.listTitleContainer}>
                 <Text style={styles.listTitle}>DANH SÁCH MÓN ĂN ĐANG CHỜ</Text>
            </View>
            <View style={styles.noteContainer}>
              <MaterialCommunityIcons name="food" size={20} color="red"/>
              <Text style={styles.noteText}>Chờ xác nhận</Text>
              <MaterialCommunityIcons name="food" size={20} color="green"/>
              <Text style={styles.noteText}>Đang chuẩn bị</Text>
            </View>
            <FlatList
                data={dishList}
