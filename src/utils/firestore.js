...
const fetchOrderedDishesData = async () => {
  try {
    const orderedDishesDoc = await firebase
      .firestore()
      .collection('ordered_dishes')
      .doc('ordered_dishes')
      .get()
...
const fetchPreOrderData = async tableId => {
  try {
    ...
    const preorderList = tableSnapshot.exists
      ? tableSnapshot.data().preorder
      : []
...
const updateDishState = async (dishId, newState) => {
  try {
    const orderedDishesRef = firebase
      .firestore()
      .collection('ordered_dishes')
      .doc('ordered_dishes')
  ...
  try {
    ...
    if (tableDocSnapshot.exists) {
      ...
      if (tableData.preorder) {
        ...
        const indexToDelete = tableData.preorder.findIndex(
          item => item.name === preorder.name && item.phone === preorder.phone
        )
      ...
      console.log(
        `Document '${table_id}' does not exist in the 'tables' collection.`
      )
    }
  } catch (error) {
